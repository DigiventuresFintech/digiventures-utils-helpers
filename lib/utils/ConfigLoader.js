...
class ConfigLoader {
    ...
    loadJwtAuthData() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!AUTHENTICATION_ARN)
                throw new Error(`JWT credentials arn not defined. You must define the 'AUTHENTICATION_ARN' environment variable`);
            const secretManager = new SecretManager_1.SecretManager();
            try {
    ...
    mongodbCredentialsLoader() {
        return __awaiter(this, void 0, void 0, function* () {
            const MONGODB_CREDENTIALS_ARN = process.env.MONGODB_CREDENTIALS_ARN;
            if (!MONGODB_CREDENTIALS_ARN)
                throw new Error(`MongoDB credentials arn not defined. You must define the 'MONGODB_CREDENTIALS_ARN' environment variable`);
            const secretManager = new SecretManager_1.SecretManager();
            try {
                const output = yield secretManager.getSecret(MONGODB_CREDENTIALS_ARN);
    ...
    bigQueryCredentialsLoader() {
        return __awaiter(this, void 0, void 0, function* () {
            const BIGQUERY_CREDENTIALS_ARN = process.env.BIGQUERY_CREDENTIALS_ARN;
            if (!BIGQUERY_CREDENTIALS_ARN)
                throw new Error(`BigQuery credentials arn not defined. You must define the 'BIGQUERY_CREDENTIALS_ARN' environment variable`);
            const secretManager = new SecretManager_1.SecretManager();
            try {
                const output = yield secretManager.getSecret(BIGQUERY_CREDENTIALS_ARN);
